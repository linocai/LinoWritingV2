...
class PromptExpanderAgent:
    ...
    # not a one-liner) and the three memory tiers below.
    #
...
② 相关记忆切片 —— involved_characters（在场角色卡 + 近期时间线）与三层记忆：
...
2. **核连续性**：对照 recent_fulltext（最近 3 章原文）、recent_summaries
   （再往前的完整梗概）与 recent_headlines（更早的一句话大事记），核对本章
   叙述与前文是否接得上；如果发现接不上，在 chapter_directive 里提示作者，
   不要擅自改动情节。**recent_fulltext / recent_summaries / recent_headlines
   都为空时**（第一章，或还没有前文）跳过连续性核对，只做结构化 + 蒸馏。
