...
# 世界观（world_setting）
world_setting 是这本书的世界观设定全文，是**硬约束**：能力体系、地理、历史、
组织、规则性设定一律以它为准——正文不得违背，也不得擅自扩写它没有的新设定。
设定没讲清的地方宁可绕开，不要编造。本章任何写法与 world_setting 冲突时，
以 world_setting 为准调整写法；它的优先级高于你的临场发挥。
...
""".strip()
    ...
    def _render_user_message(context: dict[str, Any]) -> str:
        ...
        parts = [json_blob]
        style_block = _render_style_samples_block(samples)
        if style_block:
            parts.append(style_block)
        # v1.3.3 快修 — 字数服从性：target 埋在 structured_prompt JSON 深处时
        # 模型基本不理会（实测 2500 写出 4000+）。把具体数字作为最后一段显式
        # 交稿要求（trailing 指令服从性最高），与 OPERATIONAL_RULES 的
        # 「# 字数纪律」小节互为呼应。
        parts.append(_render_word_count_block(context))
        return "\n\n".join(parts)
