...
>
> ⚠️ 原 §5.S 写的"Dockerfile 多阶段构建 + docker-compose.prod.yml + Caddy + Fly.io/Render/Hetzner 三候选评估"**整段作废**;旧 `Backend/Dockerfile` / `Backend/docker-compose.yml` / `Backend/deploy/` 三处遗留代码 v0.8 内不维护(可保留供他人 self-host 不删,文档标"unsupported"),HZ 单一部署路径不复用它们。
...
| **域名** | `<sub>.linotsai.top`(沿用作者域名,certbot 自动签);**推荐 `lw.linotsai.top`**(短 + 不与 LinoI 品牌字重复)。其它候选:`lino.linotsai.top` / `linowriting.linotsai.top`。**open question — 作者拍** |
...
- **域名拍板**:`lw.linotsai.top` vs `lino.linotsai.top` vs `linowriting.linotsai.top`。**S-3 启动前必须先定**(certbot / Nginx site / .env CORS_ORIGINS / LinoI default backend URL 都依赖)
- **UFW**:HZ 入站只 22/80/443;LinoWriting backend 走 `127.0.0.1:8787` 本机回环,**不需要开 8787 公网端口**
- **现有 `Backend/Dockerfile` / `Backend/docker-compose*.yml` / `Backend/deploy/`**:HZ 路径不复用。**S-2 决策**:(a) 删除三处遗留(YAGNI) / (b) 保留并加 `Backend/deploy/README.md` 标 "unsupported alternative for self-hosters"。默认 (a) 简化,作者拍
...
锁后 v0.8 = **12 个 Phase**(S-1/T-1/T-2/S-2/S-3/R-1/R-2/R-3/R-4/U-1/U-2/Z),S 与 R 大部分并行,真机验收处汇合;部署目标实锚 HZ,**S-3 启动前最后一个 open question 是子域名 `lw` vs `lino` vs `linowriting`**。
