...
* **视觉验证脚手架范式（Phase 2/3 实战）**：临时 `_*Scaffold.swift`（`#if os(macOS) && DEBUG`）+ in-file mock `APIClientProtocol`（喂覆盖全状态的 mock 数据）+ `AppEnvironment` DEBUG 注入钩子 + `LinoWritingApp` `--args <preview>` 分支 + 视图 `--args chapter=/tab=/autowrite` 选择器，`open LinoI.app --args …` 真 launch + computer-use zoom 逐区核 `LinoWriting.dc.html`。流式态截不到时给视图加 `ScrollViewReader` auto-scroll（既是 UX 增益又解工程问题）。用完 `rm` 脚手架 + 还原注入钩子，`grep` 确认无残留。**computer-use 在本机的 scroll/drag 偶发误报「程序坞」拦截**（Dock 在左但右侧坐标也报）——别硬刚，改用 launch-arg 直接渲染目标态 + auto-scroll 规避。**v1.2.0 P5 起 computer-use 的 `screenshot` 在本机直接返回 nil（SCContentFilter failure）截不到模拟器**——iOS 视觉核验全程用 `xcrun simctl io booted screenshot` + `Read`，比 computer-use 稳。脚手架要喂后端数据但全 mock `APIClientProtocol`（92 方法）太重时，给目标 Store 加 `#if DEBUG seedXxxForPreview(_:)` 直接种 `private(set)` 状态，比写全协议 mock 省（reader 复用 `chapterEditorStore.chapter` 命中即零 GET，种它就够）。

## iOS UITextView 两端对齐正文（v1.2.0 P5 阅读页，踩了 4 版）

* iOS 阅读正文要 `text-align:justify`+`text-indent:2em`+`line-height:2.05`，SwiftUI `Text` 都给不了 → 必须 `UIViewRepresentable` 包 `UITextView`（`isEditable=false`/`isScrollEnabled=false` 活在 SwiftUI ScrollView 内）+ `NSAttributedString` 段落属性（`.justified`/`firstLineHeadIndent=fontSize*2`/`lineHeightMultiple=2.05`/`paragraphSpacing=fontSize*1.5`）。
* **量高唯一稳的路 = iOS 16+ representable 的 `func sizeThatFits(_ proposal: ProposedViewSize, uiView:, context:) -> CGSize?`**：`let w = proposal.width ?? …; return CGSize(width:w, height:ceil(uiView.sizeThatFits([w,∞]).height))`。**别用** macOS `ReaderView` 那套 `GeometryReader{ proxy.size.width }`+`NSLayoutManager.usedRect`+`@Binding height`——`GeometryReader` 在竖向 `ScrollView` 内 greedy 报 unbounded width，正文溢出列右缘 + 破坏 justify；`widthTracksTextView=false`+手 pin `textContainer.size`/`frame.size.width`、`intrinsicContentSize` override、`NSLayoutManager` 手测都试过全不稳（要么塌成一行、要么溢出），唯表征 API 一击命中。
* 夜间「整屏」转深 + 状态栏图标转白 = 覆盖层根 `.preferredColorScheme(theme.isDark ? .dark : .light)` + 整窗背景 `theme.background.ignoresSafeArea()`（不是只染正文区）。阅读主题/字号存 **iOS 独立 `@AppStorage` key**（`reader.ios.*`，别跟 macOS `reader.*` 共用，免 sheet 串窗口）。
